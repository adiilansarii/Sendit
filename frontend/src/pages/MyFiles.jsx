...
function MyFiles() {
  ...
  // Secure download using signed URL
  const handleDownload = async (code, id) => {
    try {
      setDownloadingId(id);
  ...
  const handleCopyLink = (code) => {
    const shareableLink = `${window.location.origin}/file/${code}`;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4">
      <h1 className="text-3xl font-black mb-8 text-gray-900">
        My <span className="text-[#FF5B5B]">History</span>
      </h1>
      ...
      <div className="grid gap-3">
        {files.length === 0 ? (
          ...
          files.map((file) => (
            <div
              key={file._id}
              className="bg-white p-4 rounded-2xl border border-gray-100 flex items-center justify-between shadow-sm hover:border-red-100 transition-all"
            >
              {/* File Info */}
                ...
                {deletingId === file._id ? (
                  <div className="flex items-center gap-2 animate-in slide-in-from-right-2 duration-200">
                    <button
                      ...
                      className="bg-[#FF5B5B] text-white p-2 rounded-lg hover:bg-red-600 transition"
                    >
                      <LuCheck size={16} />
                    </button>
                    <button
                  ...
                  <div className="flex gap-1">
                    {/* Copy Link */}
                    <button
                      onClick={() => handleCopyLink(file.code)}
                    ...
                    <button
                      onClick={() =>
                        handleDownload(file.code, file._id)
                      }
                      className="p-2.5 text-gray-400 hover:text-green-500 hover:bg-green-50 rounded-xl transition-all"
                      title="Download"
                    >
                    ...
                    <button
                      ...
                      className="p-2.5 text-gray-400 hover:text-[#FF5B5B] hover:bg-red-50 rounded-xl transition-all"
                      title="Delete"
                    >
                      <LuTrash2 size={18} />
                    </button>
